#!/usr/bin/env python3
"""
🛡️ FOCUSED COUNTER STRATEGY  
=============================

Counter strategy SPECIFICALLY for days the balanced strategy doesn't trade.
Targets the ~54% of filtered days with conservative, profitable approaches.

🎯 FOCUS AREAS:
- Low premium days (where balanced filters out)
- High volatility days (where balanced avoids)
- Simple, conservative approaches that complement the primary strategy

Author: Strategy Development Framework
Date: 2025-01-31  
Version: Focused Counter v1.0
"""
...
class FocusedCounterStrategy:
    """
    Counter strategy focused on days the balanced strategy doesn't trade
    """
    ...
    def __init__(self, cache_dir: str = "../../thetadata/cached_data"):
        ...
        # Counter Strategy Parameters - Focus on complementing balanced strategy
        self.params = {
            'strategy_type': 'focused_counter',
            'max_daily_trades': 1,
    ...
    def execute_closer_atm_put(self, spy_data: pd.DataFrame, date_str: str) -> Optional[Dict]:
        """Execute closer to ATM put for higher premium"""
        ...
        target_strike = round(spy_close - adjustment, 0)  # Only $0.25 below SPY
        ...
        if option_price is None or option_price < 0.02:
            return None
        
        # Calculate outcome
        contracts = 1
        gross_premium = option_price * 100 * contracts
        costs = 3.00 * contracts  # Commission + slippage
        ...
        if spy_close > target_strike:
            ...
            intrinsic = (target_strike - spy_close) * 100 * contracts
            final_pnl = net_premium - intrinsic - (0.50 * contracts)
            outcome = 'ASSIGNED'
        
        self.logger.info(f"🎯 CLOSER ATM PUT:")
        self.logger.info(f"   Strike: ${target_strike} (${adjustment} below SPY)")
        self.logger.info(f"   Premium: ${option_price:.3f}")
        self.logger.info(f"   Final P&L: ${final_pnl:.2f} ({outcome})")
        
        return {
            'strategy': 'closer_atm_put',
...
def main():
    ...
    strategy = FocusedCounterStrategy(cache_dir=args.cache_dir)
